...
export default function Navbar2() {
  ...
  useEffect(() => {
    // Change background image based on currentSection
    if (navbarRef.current) {
      navbarRef.current.style.backgroundImage = `url(${getImageForSection(currentSection)})`;
    }
  }, [currentSection]);

  const getImageForSection = (sectionId: string) => {
    // Define your logic to map section IDs to background images
    switch (sectionId) {
      case 'headerRef':
        return 'kopcarqrosroj.png';
      case 'trabajosRef':
        return 'kopcarqnegro.png';
      case 'nosotrosRef':
        return 'kopcarqblanco.png';
      case 'contactoRef':
        return 'kopcarqblanco.png';
    }
  };
  ...
  return (
    ...
    ${isOpen ? "absolute top-0 h-screen bg-black transition-all ease-in duration-700" : "transition-all ease-out duration-500 w-0 h-16"} `}>
      ...
      <a href='#headerRef' onClick={toggleNav}
        ...
        <div ref={navbarRef} className={`h-3/4 w-full top-0 left-0 relative rounded-br-md bg-center bg-contain bg-no-repeat`} />
      </a>
